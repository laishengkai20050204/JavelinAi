...
export default function AdminConfigConsole() {
    ...
    const load = async () => {
        ...
        try {
            ...
            const e = (data.effective ?? {}) as RuntimeCfg & { compatibility?: string };
            ...
            const mem = r.memoryMaxMessages ?? e.memoryMaxMessages;
            setMemoryMaxMessages(mem ?? "");

            setBaseUrl(r.baseUrl ?? e.baseUrl ?? "");
            setApiKeyMasked(r.apiKeyMasked ?? e.apiKeyMasked ?? null);

            const ct = r.clientTimeoutMs ?? e.clientTimeoutMs;
            setClientTimeoutMs(ct ?? "");

            const st = r.streamTimeoutMs ?? e.streamTimeoutMs;
            setStreamTimeoutMs(st ?? "");
    ...
    const diffPayload = useMemo<Record<string, unknown> | null>(() => {
        if (!runtime) return null;

        const payload: Record<string, unknown> = {};
        const push = (k: string, v: unknown, cur: unknown) => {
            if (v === "") return;
        ...
        push(
            ...
            runtime.memoryMaxMessages ?? undefined
        );
        ...
        // 仅提交与 true 不同的开关；等价于你原来的 normalized
        const normalized: Record<string, boolean> = Object.fromEntries(
        ...
        runtime, compatibility, model, toolsMaxLoops, memoryMaxMessages,
        baseUrl, clientTimeoutMs, streamTimeoutMs, newApiKey, toolToggles
    ]);

    const isDiffEmpty = useMemo(() => !diffPayload || Object.keys(diffPayload).length === 0, [diffPayload]);
    ...
    const resetForm = () => {
        ...
        const mem = runtime.memoryMaxMessages ?? effective.memoryMaxMessages;
        setMemoryMaxMessages(mem ?? "");
        ...
        const ct = runtime.clientTimeoutMs ?? effective.clientTimeoutMs;
        setClientTimeoutMs(ct ?? "");

        const st = runtime.streamTimeoutMs ?? effective.streamTimeoutMs;
        setStreamTimeoutMs(st ?? "");
                        ...
                        >
                            {showEffectiveSnap ? (
                                <Card title={t.sections.snapshots.effective}>
                                    ...
                                    <Snap k="memoryMaxMessages" v={String(effective.memoryMaxMessages ?? "-")} />
                                ...
                                <Card title={t.sections.snapshots.runtime}>
                                    ...
                                    <Snap k="memoryMaxMessages" v={String(runtime?.memoryMaxMessages ?? "-")} />
                        ...
                        <div className="grid gap-4 md:grid-cols-4">
                            ...
                            <Field label={t.fields.memoryMax}>
                                <input
                                    ...
                                    onChange={(e) => setMemoryMaxMessages(e.target.value === "" ? "" : Number(e.target.value))}
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {toolNames.map((name) => {
                                        ...
                                        return (
                                            <label
                                                key={name}
                                                className={`flex select-none items-center gap-2 rounded-full border px-3 py-1 text-sm cursor-pointer ${enabled ? "bg-green-50 border-green-300 text-green-700 dark:bg-emerald-900/30 dark:border-emerald-800 dark:text-emerald-200" : "bg-white border-slate-300 text-slate-700 dark:bg-slate-800 dark:border-slate-700 dark:text-slate-200"}`}
                                                title={name}
                                            >
                                                <input
                                                    type="checkbox"
